...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const inside = todayVisits.filter(v => v.status === 'granted').length; // Simplified for prototype
  ...
  if (isUserLoading || !mounted) return (
    <div className="flex h-[60vh] items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <p className="font-mono font-bold text-primary/40 uppercase tracking-[0.4em] text-[10px]">Synchronizing Command Center...</p>
      </div>
    </div>
  );

  return (
    <div className="space-y-6 pb-10 fluid-container">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-white border rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Server Time</span>
        ...
        <div className="flex-1 max-w-2xl relative">
          ...
          <Input 
            placeholder="Global Search (ID, Name, or Department)..." 
            className="h-12 pl-12 rounded-xl bg-slate-50 border-slate-100 font-bold text-xs uppercase focus-visible:ring-primary"
            value={searchTerm}
      ...
      {/* Tier 2: Dashboard Row: Live Presence */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-6 bg-white border rounded-2xl flex flex-col justify-between h-32 transition-all hover:border-primary/20 group">
        ...
        <div className="p-6 bg-white border rounded-2xl flex flex-col justify-between h-32 transition-all hover:border-primary/20 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase mt-1">{stats.newest?.schoolId || '---'}</span>
          </div>
        </div>
        ...
        <div className={cn("p-6 border rounded-2xl flex flex-col justify-between h-32 transition-all", stats.accuracyFlags > 0 ? "bg-red-50 border-red-100" : "bg-white")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Demog.</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Purpose</th>
                </tr>
              </thead>
        ...
        {/* Right Side (25%): Mini-Analytics & Quick Settings */}
        <aside className="lg:col-span-3 space-y-6">
          <div className="bg-white border rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              {stats.distribution.map((dept, i) => (
                <div key={i} className="flex flex-col gap-1.5">
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tight">
                    ...
                    <span className="text-primary">{Math.round((dept.count / stats.totalToday) * 100)}%</span>
                  </div>
              ...
              {stats.distribution.length === 0 && <p className="text-[9px] font-bold text-slate-400 uppercase text-center">Awaiting Entry Data...</p>}
            </div>
          </div>
          ...
          <div className="bg-white border rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Target Date Range</label>
                <div className="grid grid-cols-2 gap-2">
                  <Input type="date" className="h-9 text-[10px] rounded-lg font-bold uppercase" defaultValue={format(new Date(), 'yyyy-MM-dd')} />
                  <Input type="date" className="h-9 text-[10px] rounded-lg font-bold uppercase" defaultValue={format(new Date(), 'yyyy-MM-dd')} />
                </div>
              </div>
              <Button asChild className="w-full h-12 bg-primary hover:bg-primary/90 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-primary/10">
                <Link href="/admin/reports">Generate Audit PDF</Link>
              </Button>
            </div>
          </div>

          <div className="bg-slate-900 border-none rounded-2xl p-6 shadow-xl space-y-6">
            <h2 className="text-[10px] font-black text-white/50 uppercase tracking-widest border-b border-white/10 pb-4">Terminal Engine</h2>
            <Button asChild variant="outline" className="w-full h-12 border-white/10 bg-white/5 hover:bg-white/10 text-white rounded-xl font-black uppercase text-[10px] tracking-widest">
              <Link href="/admin/settings">Manage Schema Registry</Link>
            </Button>
          </div>
        </aside>
      </div>
