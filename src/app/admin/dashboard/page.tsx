...
export default function DashboardPage() {
  ...
  if (!mounted || isUserLoading || (user && isVisitsLoading)) return (
    <div className="flex h-[60vh] items-center justify-center">
      <div className="text-center space-y-4">
        <p className="font-mono font-black text-primary/40 uppercase tracking-[0.4em] text-[10px] animate-pulse">Syncing Institutional Record...</p>
      </div>
    </div>
  );
  ...
  if (!user) return (
    <div className="flex h-[60vh] items-center justify-center">
      <p className="font-mono font-black text-red-500/40 uppercase tracking-[0.4em] text-[10px]">Security Barrier: Staff Credentials Required</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] animate-fade-in font-body overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6 shrink-0">
        ...
        <Card className="p-4 border-none shadow-sm bg-white rounded-xl flex items-center justify-between group hover:shadow-md transition-all">
          <div className="space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Daily Registry Volume</p>
        ...
        <Card className="p-4 border-none shadow-sm bg-white rounded-xl flex items-center justify-between group hover:shadow-md transition-all">
          <div className="space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Dominant Login Protocol</p>
        ...
        <Card className="p-4 border-none shadow-sm bg-white rounded-xl flex items-center justify-between group hover:shadow-md transition-all">
          <div className="space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Active Academic Units</p>
      ...
      <div className="flex-1 px-6 pb-6 overflow-hidden flex flex-col gap-4">
        <Card className="h-[300px] p-6 border-none shadow-sm bg-white rounded-2xl flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest font-headline">Authentication Logic Utilization</h2>
        ...
        <Card className="flex-1 border-none shadow-sm bg-white rounded-2xl overflow-hidden flex flex-col">
          <div className="px-6 py-3 border-b flex justify-between items-center bg-slate-50/50">
            ...
            <Badge variant="outline" className="h-6 text-[8px] font-black uppercase tracking-widest border-primary/20 text-primary">High-Density Snapshot</Badge>
          </div>
            ...
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="bg-white sticky top-0 z-10 shadow-sm">
                <tr className="border-b">
                  ...
                  <th className="w-[150px] px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Method</th>
                    ...
                    <tr key={visit.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-3">
                        <Badge variant="outline" className={cn(
                          ...
                          {visit.authMethod === 'School ID Login' ? 'ID' : 'Email'}
                        </Badge>
                      </td>
                      <td className="px-6 py-3 truncate">
